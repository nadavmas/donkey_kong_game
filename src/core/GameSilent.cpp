...
struct SilentModeHooks : public GameModeHooks {
	std::string& resultsFile;
	std::string& stepsFile;
	std::pair<size_t, Results::ResultValue> currResult{ 0, Results::noResult };

	explicit SilentModeHooks(std::string& rf, std::string& sf) : resultsFile(rf), stepsFile(sf) {}
...
void GameSilent::run(int mode) {
	...
	while (is_running == true)
	{
		...
		if (choice == 1)
		{
			SilentModeHooks hooks(results_file, steps_file);
			GameEngine engine;
...
int GameSilent::Play(int diff_lvl, Board& board, int score, std::string& file_results, std::string& file_steps)
{
	system("cls");
